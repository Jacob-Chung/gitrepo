...
function customer_service(fid){
	...
	html 	+= '	<a target="_blank" href="http://service.meizu.com/query/repair" onclick="hideMenu(\'fwin_dialog\', \'dialog\');" ><div class="custmoner1" > </div></a>';
	html 	+= '	<a target="_blank" href="http://service.meizu.com" onclick="hideMenu(\'fwin_dialog\', \'dialog\');"  ><div class="custmoner2" > </div></a>';
	html 	+= '	<div class="cr"></div>';
	html 	+= '	<a href="javascript:void(0);" onclick="hideMenu(\'fwin_dialog\', \'dialog\');window.open(\'http://live-i.meizu.com/live800/chatClient/chatbox.jsp?companyID=8957&configID=4&enterurl=&pagereferrer=&info=&k=1\',\'_blank\',\'height=473,width=703,fullscreen=3,top=200,left=200,status=yes,toolbar=no,menubar=no,resizable=no,scrollbars=no,location=no,titlebar=no,fullscreen=no\');" ><div class="custmoner3" ></div></a>';
...
//支持反对
function supportOpFun(btn, status){
	...
	$j.ajax({
		...
		success:function(data){
			...
			if (data.code === 200) {
				if (status === 'support') {
					btnItem.toggleClass('hassupport_postlist')
				} else {
					btnItem.toggleClass('hasoppose_postlist')
				}
			}
			// if(data.type == 1){
			// 	var classN = "";
